...
public class Projectile : MonoBehaviour {
	...
	private void OnCollisionEnter (Collision collision)
	{
		...
		{
			if (reaction is IHaveProjectileReactionSurface surfaceReaction)
			{
				surfaceReaction.React(transform.position, Quaternion.LookRotation (collision.contacts [0].normal));
			}
			else
			{
				reaction.React();
